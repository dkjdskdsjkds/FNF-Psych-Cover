...
import states.freeplay.CoverState;

/**
 * Music player used for Freeplay
 */
@:access(states.freeplay.CoverState)
class MusicPlayerCover extends FlxGroup 
{
	public var instance:CoverState;
	...
	public function new(instance:CoverState)
	{
		super();
	...
	override function update(elapsed:Float)
	{
		...
		var songName:String = instance.songs[CoverState.curSelected].songName;
		...
		if (playing)
		{
			if(CoverState.vocals != null)
				CoverState.vocals.volume = (CoverState.vocals.length > FlxG.sound.music.time) ? 0.8 : 0;
			if(CoverState.opponentVocals != null)
				CoverState.opponentVocals.volume = (CoverState.opponentVocals.length > FlxG.sound.music.time) ? 0.8 : 0;

			if((CoverState.vocals != null && CoverState.vocals.length > FlxG.sound.music.time && Math.abs(FlxG.sound.music.time - CoverState.vocals.time) >= 25) ||
			(CoverState.opponentVocals != null && CoverState.opponentVocals.length > FlxG.sound.music.time && Math.abs(FlxG.sound.music.time - CoverState.opponentVocals.time) >= 25))
			{
				pauseOrResume();
	...
	{
		if (CoverState.vocals != null && CoverState.vocals.length > time)
			CoverState.vocals.time = time;
		if (CoverState.opponentVocals != null && CoverState.opponentVocals.length > time)
			CoverState.opponentVocals.time = time;
	}
	...
	public function pauseOrResume(resume:Bool = false) 
	{
		if (resume)
		{
			...
			if (CoverState.vocals != null && CoverState.vocals.length > FlxG.sound.music.time && !CoverState.vocals.playing)
				CoverState.vocals.resume();
			if (CoverState.opponentVocals != null && CoverState.opponentVocals.length > FlxG.sound.music.time && !CoverState.opponentVocals.playing)
				CoverState.opponentVocals.resume();
		}
		else 
		{
			...
			if (CoverState.vocals != null)
				CoverState.vocals.pause();
			if (CoverState.opponentVocals != null)
				CoverState.opponentVocals.pause();
		}
	}
	...
	{
		var length:Int = instance.songs[CoverState.curSelected].songName.length;
	...
	function setPlaybackRate() 
	{
		...
		if (CoverState.vocals != null)
			CoverState.vocals.pitch = playbackRate;
		if (CoverState.opponentVocals != null)
			CoverState.opponentVocals.pitch = playbackRate;
	}
