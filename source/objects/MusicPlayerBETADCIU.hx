...
import states.freeplay.BETADCIUState;

/**
 * Music player used for Freeplay
 */
@:access(states.freeplay.BETADCIUState)
class MusicPlayerBETADCIU extends FlxGroup 
{
	public var instance:BETADCIUState;
	...
	public function new(instance:BETADCIUState)
	{
		super();
	...
	override function update(elapsed:Float)
	{
		...
		var songName:String = instance.songs[BETADCIUState.curSelected].songName;
		...
		//if(FlxG.keys.justPressed.K) trace('Time: ${BETADCIUState.vocals.time}, Playing: ${BETADCIUState.vocals.playing}');
		...
		if (playing)
		{
			if(BETADCIUState.vocals != null)
				BETADCIUState.vocals.volume = (BETADCIUState.vocals.length > FlxG.sound.music.time) ? 0.8 : 0;
			if(BETADCIUState.opponentVocals != null)
				BETADCIUState.opponentVocals.volume = (BETADCIUState.opponentVocals.length > FlxG.sound.music.time) ? 0.8 : 0;

			if((BETADCIUState.vocals != null && BETADCIUState.vocals.length > FlxG.sound.music.time && Math.abs(FlxG.sound.music.time - BETADCIUState.vocals.time) >= 25) ||
			(BETADCIUState.opponentVocals != null && BETADCIUState.opponentVocals.length > FlxG.sound.music.time && Math.abs(FlxG.sound.music.time - BETADCIUState.opponentVocals.time) >= 25))
			{
				pauseOrResume();
	...
	{
		if (BETADCIUState.vocals != null && BETADCIUState.vocals.length > time)
			BETADCIUState.vocals.time = time;
		if (BETADCIUState.opponentVocals != null && BETADCIUState.opponentVocals.length > time)
			BETADCIUState.opponentVocals.time = time;
	}
	...
	public function pauseOrResume(resume:Bool = false) 
	{
		if (resume)
		{
			...
			if (BETADCIUState.vocals != null && BETADCIUState.vocals.length > FlxG.sound.music.time && !BETADCIUState.vocals.playing)
				BETADCIUState.vocals.resume();
			if (BETADCIUState.opponentVocals != null && BETADCIUState.opponentVocals.length > FlxG.sound.music.time && !BETADCIUState.opponentVocals.playing)
				BETADCIUState.opponentVocals.resume();
		}
		else 
		{
			...
			if (BETADCIUState.vocals != null)
				BETADCIUState.vocals.pause();
			if (BETADCIUState.opponentVocals != null)
				BETADCIUState.opponentVocals.pause();
		}
	}
	...
	{
		var length:Int = instance.songs[BETADCIUState.curSelected].songName.length;
	...
	function setPlaybackRate() 
	{
		...
		if (BETADCIUState.vocals != null)
			BETADCIUState.vocals.pitch = playbackRate;
		if (BETADCIUState.opponentVocals != null)
			BETADCIUState.opponentVocals.pitch = playbackRate;
	}
