...
import haxe.Http;
import states.downloads.DownloadMenuState;
import backend.MusicBeatState;

using StringTools;
//** Based on Some Psych Unused Idea :/ by Magnumsrt**/
class ModDownloadState extends MusicBeatState
{
    ...
    override public function create()
    {
        ...
        var tabs = [{name: 'Input Link Here', label: 'Input Link Here'},];
        ...
        var coolButton = new FlxButton(0, 0, "Install", function()
		{
    ...
    override public function update(elapsed:Float)
    {
        if (coolParsed != null)
        {
            ...
            if (completed)
            {
                if (FileSystem.exists(coolFile.replace('.zip', '')) && FileSystem.readDirectory(coolFile.replace('.zip', '') + '/') != null)
                {
            ...
            if (FileSystem.exists(coolFile.replace('.zip', '')) && finished && !fileConfirmed)
            {
                for (folder in FileSystem.readDirectory(coolFile.replace('.zip', '') + '/'))
                {
                    ...
                    {
                        if (FileSystem.readDirectory('mods/$folder/') != null && !FileSystem.readDirectory('mods/$folder/').contains('pack.json'))
                        {
		...
		{
			MusicBeatState.switchState(new DownloadMenuState());
		}
