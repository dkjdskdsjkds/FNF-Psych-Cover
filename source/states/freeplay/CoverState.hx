...
import objects.MusicPlayerCover;
...
class CoverState extends MusicBeatState
{
	...
	public var playerCover:MusicPlayerCover;
	...
	override function create()
	{
		...
		DiscordClient.changePresence("In Cover Song Menu", null);
		#end
		...
		playerCover = new MusicPlayerCover(this);
		add(playerCover);
	...
	{
		PlayState.isCover = true; //gotta move this cuz of the format thing
		...
		if (controls.BACK)
		{
			if (playerCover.playingMusic)
			{
				...
				playerCover.playingMusic = false;
				playerCover.switchPlayMusic();
		...
		if(FlxG.keys.justPressed.CONTROL && !playerCover.playingMusic)
		{
			persistentUpdate = false;
		...
		{
			if(instPlaying != curSelected && !playerCover.playingMusic)
			{
				destroyFreeplayVocals();
				FlxG.sound.music.volume = 0;
				...
				playerCover.playingMusic = true;
				playerCover.curTime = 0;
				playerCover.switchPlayMusic();
				playerCover.pauseOrResume(true);
			}
			else if (instPlaying == curSelected && playerCover.playingMusic)
			{
				playerCover.pauseOrResume(!playerCover.playing);
			}
		}
		else if (controls.ACCEPT && !playerCover.playingMusic)
		{
			persistentUpdate = false;
		...
		else if(controls.RESET && !playerCover.playingMusic)
		{
			persistentUpdate = false;
